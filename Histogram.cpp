...
void Histogram::computeHstgrms(QImage & img)
{
    if (!img.isNull())
    {
...
void Histogram::paintEvent(QPaintEvent * event)
{
    ...
    int step = 70;              // distance between histograms
    ...
    if (bwHstgrm[256] != -1)
        ...
        painter.drawText(xBase, yBase-height/2+5, "Can't load the gray levels histogram.");
    
    if (redHstgrm[256] != -1)
        drawRedHstgrm(xBase, yBase+step+height+1, height);
    else
        painter.drawText(xBase, yBase+step+height+1-height/2+5, "Can't load the red component histogram.");
    
    if (greenHstgrm[256] != -1)
        drawGreenHstgrm(xBase, yBase+2*(step+height+1), height);
    else
        painter.drawText(xBase, yBase+2*(step+height+1)-height/2+5, "Can't load the green component histogram.");
    
    if (blueHstgrm[256] != -1)
        drawBlueHstgrm(xBase, yBase+3*(step+height+1), height);
    else
        painter.drawText(xBase, yBase+3*(step+height+1)-height/2+5, "Can't load the blue component histogram.");
}
...
void Histogram::drawBwHstgrm(int xBase, int yBase, int height)
{
    ...
    painter.drawText(xBase, yBase+25, "black");
    painter.drawText(xBase+220, yBase+25, "white");
    
    painter.setPen(Qt::black);
    
    painter.drawText(xBase+40, yBase-height-10, "GRAY LEVELS HISTOGRAM");
    
    painter.drawText(xBase+100, yBase+15, "Intensity");
    painter.drawText(xBase-84, yBase-height/2+5, "Pixels count");
...
void Histogram::drawRedHstgrm(int xBase, int yBase, int height)
{
    ...
    painter.drawText(xBase, yBase+25, "dark");
    painter.drawText(xBase+225, yBase+25, "light");
    
    painter.setPen(Qt::black);
    
    painter.drawText(xBase+25, yBase-height-10, "RED COMPONENT HISTOGRAM");
...
void Histogram::drawGreenHstgrm(int xBase, int yBase, int height)
{
    ...
    painter.drawText(xBase+15, yBase-height-10, "GREEN COMPONENT HISTOGRAM");
...
void Histogram::drawBlueHstgrm(int xBase, int yBase, int height)
{
    ...
    painter.drawText(xBase+20, yBase-height-10, "BLUE COMPONENT HISTOGRAM");
